...
- Generated by wxSmith (visual designer tool)
- Auto-generates from template with controls for each parameter
- Allows real-time parameter preview
- **IMPORTANT**: When modifying UI controls, always update both the `.wxs` file AND the generated `.cpp` file (see wxSmith section below)
...
### wxSmith UI Designer System

**Overview**:
wxSmith is a visual GUI designer integrated with Code::Blocks that generates wxWidgets C++ code from XML configuration files. xLights uses wxSmith extensively for dialog and panel layouts.

**Key Concepts**:

**Source Files**:
- `.wxs` files - wxSmith configuration files (XML format) that define the UI layout
- Located in `xLights/wxsmith/` directory
- Contains widget definitions, properties, layout information, and event handlers

**Generated Code**:
- `.h` and `.cpp` files - Auto-generated C++ code from `.wxs` files
- Contains GUI initialization code in constructor
- Marked with special comment blocks indicating auto-generated sections
- **NEVER manually edit generated code blocks** - changes will be overwritten

**Critical Rule - Dual File Updates**:
When modifying UI controls (sliders, text boxes, buttons, etc.), you MUST update BOTH files:

1. **Update the `.wxs` file** - The source configuration file
2. **Update the generated `.cpp` file** - The actual C++ code

If only one is updated, they will fall out of sync and cause issues during code review and future regeneration.

**Example - WavePanel Slider Minimum Value**:

`.wxs` file location: `/xLights/xLights/wxsmith/WavePanel.wxs`
```xml
<object class="wxSlider" name="ID_SLIDER_Number_Waves" subclass="BulkEditSlider"
        variable="Slider_Number_Waves" member="yes">
    <value>900</value>
    <min>1</min>          <!-- Change minimum value here -->
    <max>3600</max>
</object>
```

Generated `.cpp` file location: `/xLights/xLights/effects/WavePanel.cpp`
```cpp
// Auto-generated code block - DO NOT MANUALLY EDIT
Slider_Number_Waves = new BulkEditSlider(this, ID_SLIDER_Number_Waves,
    900,    // initial value
    1,      // min value - must match .wxs file
    3600,   // max value
    wxDefaultPosition, wxDefaultSize, 0, wxDefaultValidator,
    _T("ID_SLIDER_Number_Waves"));
```

**Common UI Controls in .wxs Files**:
- `<object class="wxSlider">` - Slider controls
  - Properties: `<value>`, `<min>`, `<max>`
- `<object class="wxTextCtrl">` - Text input boxes
  - Properties: `<value>`, `<maxlength>`, `<size>`
- `<object class="wxChoice">` - Dropdown menus
  - Properties: `<content>`, `<selection>`
- `<object class="wxCheckBox">` - Checkboxes
  - Properties: `<label>`, `<checked>`
- `<object class="wxButton">` - Buttons
  - Properties: `<label>`, `<handler>` (event binding)
- `<object class="wxBitmapButton">` - Icon buttons
  - Properties: `<bitmap>`, `<size>`, `<handler>`

**Layout Management**:
- `<object class="wxFlexGridSizer">` - Flexible grid layout
  - Properties: `<cols>`, `<rows>`, `<growablecols>`, `<growablerows>`
- `<object class="wxBoxSizer">` - Linear box layout
- `<object class="sizeritem">` - Container for widgets in sizers
  - Properties: `<flag>` (alignment/expansion), `<border>`, `<option>` (proportion)

**Event Handlers**:
```xml
<handler function="OnLockButtonClick" entry="EVT_BUTTON" />
```
- Links UI events to C++ member functions
- Generated code creates event table entries

**Workflow for UI Changes**:

1. **Planning**: Identify the control to modify (e.g., slider min/max, text box size)
2. **Locate .wxs file**: Find corresponding file in `xLights/wxsmith/`
3. **Edit .wxs file**: Update the XML property (e.g., `<min>1</min>`)
4. **Locate generated .cpp file**: Usually in `xLights/effects/` or main `xLights/` directory
5. **Edit .cpp file**: Find the auto-generated code block and update matching parameter
6. **Verify consistency**: Ensure both files have identical values
7. **Build and test**: Compile to verify changes work correctly

**Auto-Generated Code Markers**:
Look for these comment patterns in `.cpp` files:
```cpp
//(*Initialize(WavePanel)
// ... auto-generated code ...
//*)
```
These sections are regenerated by wxSmith if you edit the `.wxs` file in Code::Blocks.

**Best Practices**:
- Always update both `.wxs` and `.cpp` files together
- Use Code::Blocks wxSmith editor when possible (updates both automatically)
- If manually editing, double-check both files for consistency
- Test UI changes thoroughly after modifications
- Be aware that regenerating from wxSmith will overwrite manual changes to generated blocks

**Common Files Using wxSmith**:
- Effect panels: `BarsPanel.wxs`, `WavePanel.wxs`, `FirePanel.wxs`, etc.
- Dialogs: `CustomModelDialog.wxs`, `ControllerModelDialog.wxs`, etc.
- Main UI: `MainSequencer.wxs`, `LayoutPanel.wxs`, `EffectsPanel.wxs`
