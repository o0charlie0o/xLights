...
{
    size_t totalBytes = static_cast<size_t>(numFrames) * numChannels;
    _data.resize(totalBytes, 0);
}

NativeSequenceData::~NativeSequenceData() = default;
...
uint8_t* NativeSequenceData::getFrame(uint32_t frameIndex)
{
    ...
    return _data.data() + static_cast<size_t>(frameIndex) * _numChannels;
}
...
{
    if (frame >= _numFrames || channel >= _numChannels) {
        return;
    }
...
void NativeSequenceData::zeroFrame(uint32_t frameIndex)
{
    ...
    std::memset(_data.data() + static_cast<size_t>(frameIndex) * _numChannels, 0, _numChannels);
}
