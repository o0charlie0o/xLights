...
EffectsGrid::EffectsGrid(MainSequencer* parent, wxWindowID id, const wxPoint& pos, const wxSize& size,
    ...
    // Note: wxCursor does not support Retina/HiDPI on macOS, so cursors will appear non-Retina
    // This is a limitation of wxWidgets - native NSCursor would be needed for Retina support
    ...
    // Try to load sparkles cursor from PNG
    wxImage sparklesCursorImg;
    if (sparklesCursorImg.LoadFile(resourcesDir + "/cursor_sparkle.png", wxBITMAP_TYPE_PNG)) {
        sparklesCursorImg.SetOption(wxIMAGE_OPTION_CUR_HOTSPOT_X, 16);
        sparklesCursorImg.SetOption(wxIMAGE_OPTION_CUR_HOTSPOT_Y, 16);
        mCursorSparkles = wxCursor(sparklesCursorImg);
    }
...
void EffectsGrid::rightClick(wxMouseEvent& event) {
    ...
    if (element->GetType() != ElementType::ELEMENT_TYPE_TIMING) {
        // Get the effect under the cursor (if any) for operations that can work on right-clicked effect
        int effectIndex;
        HitLocation selectionType;
        int clickTimeMS = mTimeline->GetAbsoluteTimeMSfromPosition(event.GetX());
        _rightClickEffect = GetEffectAtRowAndTime(mSelectedRow, clickTimeMS, effectIndex, selectionType);
        ...
        // Duplicate submenu
        wxMenu* mnuDuplicate = new wxMenu();
        wxMenuItem* menu_duplicate = mnuDuplicate->Append(ID_GRID_MNU_DUPLICATE_EFFECT, "Duplicate");
        bool paste_by_cell = ((MainSequencer*)mParent)->PasteByCellActive();
        if (mSelectedEffect == nullptr) {
            menu_duplicate->Enable(false);
        }
        mnuDuplicate->AppendSeparator();
        wxMenuItem* menu_duplicate_right = mnuDuplicate->Append(ID_GRID_MNU_DUPLICATE_RIGHT, "Duplicate Right");
        if (mSelectedEffect == nullptr && !mCellRangeSelected) {
            menu_duplicate_right->Enable(false);
        }

        wxMenuItem* menu_duplicate_left = mnuDuplicate->Append(ID_GRID_MNU_DUPLICATE_LEFT, "Duplicate Left");
        if (mSelectedEffect == nullptr && !mCellRangeSelected) {
            menu_duplicate_left->Enable(false);
        }

        wxMenuItem* menu_duplicate_up = mnuDuplicate->Append(ID_GRID_MNU_DUPLICATE_UP, "Duplicate Up");
        if (mSelectedEffect == nullptr && !mCellRangeSelected) {
            menu_duplicate_up->Enable(false);
        }

        wxMenuItem* menu_duplicate_down = mnuDuplicate->Append(ID_GRID_MNU_DUPLICATE_DOWN, "Duplicate Down");
        if (mSelectedEffect == nullptr && !mCellRangeSelected) {
            menu_duplicate_down->Enable(false);
        }
        mnuDuplicate->Connect(wxEVT_COMMAND_MENU_SELECTED, (wxObjectEventFunction)&EffectsGrid::OnGridPopup, nullptr, this);
        mnuLayer.AppendSubMenu(mnuDuplicate, "Duplicate");
        ...
        // Determine which effect(s) to work with for symbol operations
        // Priority: right-clicked effect, then selected effect, then multiple selected effects
        ...
        // Check if any effect (right-clicked or selected) is linked to a symbol
        ...
        // Link to Symbol submenu - available for single or multiple effects
...
void EffectsGrid::mouseMoved(wxMouseEvent& event) {
    ...
    if (mResizing) {
        ...
        if (mResizingMode == EFFECT_RESIZE_SMART_BRIGHTNESS || mResizingMode == EFFECT_RESIZE_SMART_SPARKLES) {
        ...
        if (effectLayer->IsFixedTimingLayer()) {
            ...
            if (altDown && yPos >= 0) {
                ...
                if (position < leftEdgeEnd) {
                    ...
                    // CENTER - Check modifier keys to determine which property to adjust
                    if (shiftDown) {
                        // Alt+Shift - Sparkles
                        selectionType = HitLocation::SMART_SPARKLES;
                    } else {
                        // Alt only - Brightness
...
void EffectsGrid::mouseDown(wxMouseEvent& event) {
    ...
    if (mResizingMode != EFFECT_RESIZE_NO) {
        if (selectedEffect != nullptr) {
            ...
            if (mResizingMode == EFFECT_RESIZE_SMART_FADE || mResizingMode == EFFECT_RESIZE_SMART_BRIGHTNESS || mResizingMode == EFFECT_RESIZE_SMART_SPARKLES) {
                ...
                } else if (mResizingMode == EFFECT_RESIZE_SMART_SPARKLES) {
                    // CRITICAL: Stop the EffectSettingsTimer to prevent it from overwriting our changes
                    xlights->EffectSettingsTimer.Stop();

                    std::string sparklesStr = settings.Get("C_SLIDER_SparkleFrequency", "0");
                    // If sparkles doesn't exist in SettingsMap, initialize it
                    if (sparklesStr.empty()) {
                        sparklesStr = "0";
                        selectedEffect->SetSetting("C_SLIDER_SparkleFrequency", "0");
                    }
                    mSmartToolInitialValue = wxAtoi(sparklesStr);
                }
            }
...
void EffectsGrid::mouseReleased(wxMouseEvent& event) {
    ...
    if (mDragging && xlights->IsACActive()) {
        ...
        if (mResizing && (mResizingMode == EFFECT_RESIZE_SMART_FADE || mResizingMode == EFFECT_RESIZE_SMART_BRIGHTNESS || mResizingMode == EFFECT_RESIZE_SMART_SPARKLES)) {
            // Only call RaiseSelectedEffectChanged for fade adjustments
            // For brightness and sparkles: RaiseSelectedEffectChanged resets the value back to defaults,
            // so we skip it. The value is already saved in SettingsMap and rendering
            // is already triggered by sendRenderDirtyEvent() in AdjustEffectBrightness/AdjustEffectSparkles
            if (mResizingMode == EFFECT_RESIZE_SMART_FADE && mSelectedEffect != nullptr) {
                RaiseSelectedEffectChanged(mSelectedEffect, false, false);
            }

            // Clear the status bar that was showing brightness or sparkles value during drag
            if ((mResizingMode == EFFECT_RESIZE_SMART_BRIGHTNESS || mResizingMode == EFFECT_RESIZE_SMART_SPARKLES) && mSelectedEffect != nullptr) {
                // Clear the status text (use same field/format as during drag)
...
void EffectsGrid::AdjustEffectSparkles(int yPosition) {
    // Calculate drag delta from initial Y position
    // Negative because drag up = increase value (screen Y increases downward)
    int dragDelta = mSmartToolDragStartY - yPosition;

    // Convert to sparkles change (1 pixel = 1 sparkle unit, range 0-200)
    float sparklesDelta = dragDelta / 1.0f;

    // Calculate new sparkles value
    float newSparkles = mSmartToolInitialValue + sparklesDelta;

    // Clamp to valid range (0 to 200)
    newSparkles = std::max(0.0f, std::min(200.0f, newSparkles));

    // Update status bar to show current sparkles value during drag
    xlights->SetStatusText(wxString::Format("Sparkles: %d", (int)newSparkles));
        ...
        if (el != nullptr) {
            for (int ef = 0; ef < el->GetEffectCount(); ef++) {
                ...
                if (effect != nullptr && effect->GetSelected() != EFFECT_NOT_SELECTED) {
                    ...
                    // Check if value curve is active
                    if (!settings.Get("C_VALUECURVE_SparkleFrequency", "").empty() &&
                        settings.Get("C_VALUECURVE_SparkleFrequency", "").find("Active=TRUE") != std::string::npos) {
                        // Value curve is active - skip sparkles adjustment
                        // Smart Tool doesn't support value curves
                        continue;
                    } else {
                        // Use regular sparkles slider
                        effect->SetSetting("C_SLIDER_SparkleFrequency", std::to_string((int)newSparkles));
                    }
                }
            }
        }
    ...
    // CRITICAL: Update the UI controls to match the new sparkles value
    // This prevents the timer from reading the old value from the UI and overwriting our change
    ColorPanel* colorPanel = xlights->GetColorPanel();
    if (colorPanel != nullptr && colorPanel->Slider_SparkleFrequency != nullptr) {
        colorPanel->Slider_SparkleFrequency->SetValue((int)newSparkles);
    }
    if (colorPanel != nullptr && colorPanel->txtCtrlSparkleFreq != nullptr) {
        colorPanel->txtCtrlSparkleFreq->SetValue(std::to_string((int)newSparkles));
    }
...
void EffectsGrid::RunMouseOverHitTests(int rowIndex, int x, int y, bool altDown, bool shiftDown) {
    ...
    if (eff != nullptr) {
        ...
        case HitLocation::SMART_BRIGHTNESS:
            ...
            break;
        case HitLocation::SMART_SPARKLES:
...
void EffectsGrid::DuplicateEffectRight() {
    ...
    if (mCellRangeSelected) {
        ...
        if (paste_by_cell) {
            ...
            fprintf(stderr, "DuplicateEffectRight: Range selected (paste by cell), cols %d-%d, rows %d-%d\n",
            ...
            for (int row = startRow; row <= endRow; row++) {
                ...
                for (int col = startCol; col <= endCol; col++) {
                    ...
                    // Check if the effect starts in this cell (it may extend beyond the cell)
                    if (source_effect && source_effect->GetStartTimeMS() >= cell_start &&
                        source_effect->GetStartTimeMS() < cell_end) {

                        // Duplicate maintaining the original effect's duration
                        long effect_duration = source_effect->GetEndTimeMS() - source_effect->GetStartTimeMS();
                        long newstart = mTimeline->RoundToMultipleOfPeriod(target_timing->GetStartTimeMS(), mSequenceElements->GetFrequency());
                        long newEnd = mTimeline->RoundToMultipleOfPeriod(newstart + effect_duration, mSequenceElements->GetFrequency());
            ...
            // Paste by time mode - duplicate effects maintaining their relative positions
            fprintf(stderr, "DuplicateEffectRight: Range selected (paste by time), rows %d-%d\n",
                startRow, endRow);

            // Find the rightmost effect in the selection to determine where to place duplicates
            long range_start_time = LONG_MAX;
            long range_end_time = 0;
            ...
            for (int row = startRow; row <= endRow; row++) {
                ...
                fprintf(stderr, "  Row %d: checking %d effects\n", row, layer->GetEffectCount());
                for (int i = 0; i < layer->GetEffectCount(); i++) {
                    Effect* eff = layer->GetEffect(i);
                    if (eff && eff->GetSelected() != EFFECT_NOT_SELECTED) {
                        selected_count++;
                        fprintf(stderr, "    Found selected effect at %ldms-%ldms\n",
                            eff->GetStartTimeMS(), eff->GetEndTimeMS());
                        if (eff->GetStartTimeMS() < range_start_time) {
                            range_start_time = eff->GetStartTimeMS();
                        }
                        if (eff->GetEndTimeMS() > range_end_time) {
                            range_end_time = eff->GetEndTimeMS();
                        }
                    }
                }
            }

            fprintf(stderr, "  Selected effects: %d, range: %ldms-%ldms\n",
                selected_count, range_start_time, range_end_time);

            if (range_end_time == 0 || selected_count == 0) {
                fprintf(stderr, "  No selected effects found in range\n");
                return; // No effects found in range
            }

            long range_width = range_end_time - range_start_time;

            // Collect all effects to duplicate first (to avoid duplicating newly created effects)
            struct EffectToDuplicate {
                Effect* source;
                EffectLayer* layer;
                long newstart;
                long newEnd;
            };
            std::vector<EffectToDuplicate> effects_to_duplicate;
            ...
            for (int row = startRow; row <= endRow; row++) {
                ...
                for (int i = 0; i < layer->GetEffectCount(); i++) {
                    Effect* source_effect = layer->GetEffect(i);
                    if (source_effect && source_effect->GetSelected() != EFFECT_NOT_SELECTED) {
                        long effect_start = source_effect->GetStartTimeMS();
                        long effect_end = source_effect->GetEndTimeMS();
                        long effect_duration = effect_end - effect_start;
                        long offset_from_range_start = effect_start - range_start_time;

                        // Place duplicate after the range end, maintaining relative position
                        long newstart = mTimeline->RoundToMultipleOfPeriod(range_end_time + offset_from_range_start, mSequenceElements->GetFrequency());
                        long newEnd = mTimeline->RoundToMultipleOfPeriod(newstart + effect_duration, mSequenceElements->GetFrequency());

                        effects_to_duplicate.push_back({source_effect, layer, newstart, newEnd});
                    }
                }
            }

            // Now duplicate all the collected effects
            for (const auto& etd : effects_to_duplicate) {
                fprintf(stderr, "    Duplicating effect from %ldms-%ldms to %ldms-%ldms\n",
                    etd.source->GetStartTimeMS(), etd.source->GetEndTimeMS(), etd.newstart, etd.newEnd);

                if (!etd.layer->HasEffectsInTimeRange(etd.newstart, etd.newEnd)) {
                    Effect* newef = etd.layer->AddEffect(0, xlights->GetEffectManager().GetEffectName(etd.source->GetEffectIndex()),
                        etd.source->GetSettingsAsString(), etd.source->GetPaletteAsString(),
                        etd.newstart, etd.newEnd, EFFECT_SELECTED, false);
                    mSequenceElements->get_undo_mgr().CaptureAddedEffect(etd.layer->GetParentElement()->GetName(), etd.layer->GetIndex(), newef->GetID());
                    fprintf(stderr, "      Effect duplicated successfully\n");
                } else {
                    fprintf(stderr, "      Target range already has effects\n");
                }
            }
        }
...
void EffectsGrid::DuplicateEffectLeft() {
    ...
    if (mCellRangeSelected) {
        ...
        fprintf(stderr, "DuplicateEffectLeft: Range selected (paste by cell), cols %d-%d, rows %d-%d\n",
            startCol, endCol, startRow, endRow);
        ...
        for (int row = startRow; row <= endRow; row++) {
            ...
            // Paste by time mode - duplicate effects maintaining their relative positions
            fprintf(stderr, "DuplicateEffectLeft: Range selected (paste by time), rows %d-%d\n",
                startRow, endRow);

            // Find the leftmost and rightmost effects in the selection
            long range_start_time = LONG_MAX;
            long range_end_time = 0;
            int selected_count = 0;
            ...
            if (range_start_time == LONG_MAX || selected_count == 0) {
                fprintf(stderr, "  No selected effects found in range\n");
                return; // No effects found in range
            }
            ...
            // Check if we can duplicate left (must not go before time 0)
            if (range_start_time - range_width < 0) {
                fprintf(stderr, "  Cannot duplicate left - would go before time 0\n");
                return;
            }

            // Collect all effects to duplicate first
            struct EffectToDuplicate {
                Effect* source;
                EffectLayer* layer;
                long newstart;
            ...
            for (int row = startRow; row <= endRow; row++) {
                ...
                for (int i = 0; i < layer->GetEffectCount(); i++) {
                    ...
                    if (source_effect && source_effect->GetSelected() != EFFECT_NOT_SELECTED) {
                        ...
                        // Place duplicate before the range start, maintaining relative position
                        long newstart = mTimeline->RoundToMultipleOfPeriod(range_start_time - range_width + offset_from_range_start, mSequenceElements->GetFrequency());
                        long newEnd = mTimeline->RoundToMultipleOfPeriod(newstart + effect_duration, mSequenceElements->GetFrequency());
...
void EffectsGrid::DuplicateEffectUp() {
    ...
    if (mCellRangeSelected) {
        ...
        if (paste_by_cell) {
            ...
            fprintf(stderr, "DuplicateEffectUp: Range selected (paste by cell), cols %d-%d, rows %d-%d\n",
                startCol, endCol, startRow, endRow);
            ...
            // Paste by time mode - duplicate effects maintaining their time positions
            fprintf(stderr, "DuplicateEffectUp: Range selected (paste by time), rows %d-%d\n",
                startRow, endRow);

            // Collect all effects to duplicate first
            struct EffectToDuplicate {
                Effect* source;
                EffectLayer* target_layer;
                long newstart;
                long newEnd;
            };
            std::vector<EffectToDuplicate> effects_to_duplicate;
            ...
            for (int row = startRow; row <= endRow; row++) {
                ...
                for (int i = 0; i < layer->GetEffectCount(); i++) {
                    ...
                    if (source_effect && source_effect->GetSelected() != EFFECT_NOT_SELECTED) {
                        ...
                        long newstart = mTimeline->RoundToMultipleOfPeriod(effect_start, mSequenceElements->GetFrequency());
                        long newEnd = mTimeline->RoundToMultipleOfPeriod(effect_end, mSequenceElements->GetFrequency());

                        effects_to_duplicate.push_back({source_effect, target_layer, newstart, newEnd});
                    }
                }
            }
            ...
            // Now duplicate all the collected effects
            for (const auto& etd : effects_to_duplicate) {
                if (!etd.target_layer->HasEffectsInTimeRange(etd.newstart, etd.newEnd)) {
                    Effect* newef = etd.target_layer->AddEffect(0, xlights->GetEffectManager().GetEffectName(etd.source->GetEffectIndex()),
                        etd.source->GetSettingsAsString(), etd.source->GetPaletteAsString(),
                        etd.newstart, etd.newEnd, EFFECT_SELECTED, false);
                    mSequenceElements->get_undo_mgr().CaptureAddedEffect(etd.target_layer->GetParentElement()->GetName(), etd.target_layer->GetIndex(), newef->GetID());
                }
            }
        }
...
void EffectsGrid::DuplicateEffectDown() {
    ...
    if (mCellRangeSelected) {
        ...
        if (paste_by_cell) {
            ...
            // Paste by time mode - duplicate effects maintaining their time positions
            fprintf(stderr, "DuplicateEffectDown: Range selected (paste by time), rows %d-%d\n",
                startRow, endRow);
            ...
            for (int row = startRow; row <= endRow; row++) {
                ...
                int targetRow = row + rangeHeight;
                if (targetRow >= mSequenceElements->GetVisibleRowInformationSize()) continue;
...
{
    // Use right-clicked effect if available, otherwise use selected effect
    ...
    // Create the symbol from the target effect
    EffectSymbol* symbol = symbolMgr.CreateSymbol(symbolName, targetEffect);
    if (symbol != nullptr) {
        // Link the target effect to the new symbol
...
void EffectsGrid::UnlinkEffectFromSymbol()
{
    ...
    // Include right-clicked effect if not already in the list
    if (_rightClickEffect != nullptr) {
        bool found = false;
        for (const auto& ef : selectedEffects) {
            if (ef == _rightClickEffect) {
                found = true;
                break;
            }
        }
        if (!found) {
            selectedEffects.push_back(_rightClickEffect);
        }
    }
...
void EffectsGrid::LinkEffectToSymbol(int symbolIndex)
{
    ...
    // Collect all effects to link: right-clicked effect, selected effects, and mSelectedEffect
    std::vector<Effect*> effectsToLink;

    // Add right-clicked effect if present
    if (_rightClickEffect != nullptr) {
        effectsToLink.push_back(_rightClickEffect);
    }

    // Add all selected effects
    auto selectedEffects = GetSelectedEffects();
    for (Effect* eff : selectedEffects) {
        if (eff != nullptr) {
            bool found = false;
            ...
            if (!found) {
                effectsToLink.push_back(eff);
            }
        }
    }

    // Add mSelectedEffect if not already included
    if (mSelectedEffect != nullptr) {
        bool found = false;
        for (Effect* existing : effectsToLink) {
            if (existing == mSelectedEffect) {
                found = true;
                break;
            }
        }
        if (!found) {
            effectsToLink.push_back(mSelectedEffect);
        }
    }
    ...
    // Link all collected effects to the symbol
